  ...
  class Conv {
   ...
   private:
    ...
    inline void open(string in) {
      ...
      for (int i = 1; i < m_img->height(); i++) {
        m_img->data()[i] = m_img->data()[0] + 3 * m_img->width() * i;
      }

      while (cinfo.output_scanline < cinfo.output_height) {
        jpeg_read_scanlines(&cinfo, m_img->data() + cinfo.output_scanline, cinfo.output_height - cinfo.output_scanline);
      }
